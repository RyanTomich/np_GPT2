...
def transformer_gpt2_model(model_name):
    ...
    )  # loading gpt2 from transformers library
    gpt2_tokenizer = GPT2Tokenizer.from_pretrained(
        model_name, clean_up_tokenization_spaces=True
    )  # loading gpt2 tokenizer from transformers library
    return gpt2, gpt2_tokenizer


def transformer_gpt2_inference(
    prompt, model, tokenizer, do_sample=False, temperature=1, max_length=100
):
...
class NpGPT2:
    def __init__(
        ...
        self.key_cache = [
            np.empty((0, self.embedding_size)) for _ in range(decode_blocks)
        ]
        self.value_cache = [
            np.empty((0, self.embedding_size)) for _ in range(decode_blocks)
        ]
    ...
    def get_model_architecture(self, model):
        ...
        for i in range(36):
            ...
            for name, param in state_dict.items():
                ans = param.numpy()
                if (
                    "h." + str(i) + "." in name and i == 0
                ):  # each h.# refers to a transformer block
    ...
    def matrix_self_attn(self, emb, block_num, mask=None):
        ...
        def split_heads(x):
            *start, m = x.shape
            a = np.reshape(
                x, start + [self.attn_heads, m // self.attn_heads]
            )  # matrix to tensor with heads dimention
            trace.file_write(
                f"split_heads", in_size=(x.shape, np.transpose(a, [1, 0, 2]).shape)
            )
        ...
        def merge_heads(x):
            ...
            *start, a, b = x.shape
            trace.file_write(
                f"merge_heads", in_size=(x.shape, np.reshape(x, start + [a * b]).shape)
            )
            return np.reshape(x, start + [a * b])

        # attn
        attn_weights = self.parameters[
            "transformer.h." + str(block_num) + ".attn.c_attn.weight"
        ]
        attn_bias = self.parameters[
            "transformer.h." + str(block_num) + ".attn.c_attn.bias"
        ]
        ...
        weights = self.parameters[
            "transformer.h." + str(block_num) + ".attn.c_proj.weight"
        ]
    ...
    def mlp(self, emb, block_num):
        ...
        """
        weights = self.parameters[
            "transformer.h." + str(block_num) + ".mlp.c_fc.weight"
        ]
        ...
        weights = self.parameters[
            "transformer.h." + str(block_num) + ".mlp.c_proj.weight"
        ]
    ...
    def next_token(self, tok):
        ...
        # Logit selection
        logit_wrappers = {"greedy": lambda x: np.argmax(x[-1]), "top_k": self.top_k}
